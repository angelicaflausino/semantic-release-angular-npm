  ...
  await t.notThrowsAsync(
    t.context.m.verifyConditions(
      { npmPublish: false },
      {
        cwd,
        env: {},
        options: { publish: ["semantic-release-angular-npm"] },
...
test("Throw SemanticReleaseError Array if config option are not valid in verifyConditions", async (t) => {
      ...
      await t.throwsAsync(
        t.context.m.verifyConditions(
          {},
          {
            ...
            env: {},
            options: {
              publish: ["@semantic-release/github", { path: "semantic-release-angular-npm", npmPublish, tarballDir, pkgRoot, ngPackageFilePath }],
            },
            stdout: t.context.stdout,
            stderr: t.context.stderr,
            logger: t.context.logger,
  ...
  t.is(errors[4].code, "EINVALIDNGPACKAGEFILEPATH");
});
      ...
      await t.throwsAsync(
        t.context.m.publish(
          { npmPublish, tarballDir, pkgRoot },
          {
            cwd,
            env: {},
            options: { publish: ["@semantic-release/github", "semantic-release-angular-npm"] },
            nextRelease: { version: "1.0.0" },
            stdout: t.context.stdout,
            stderr: t.context.stderr,
            logger: t.context.logger,

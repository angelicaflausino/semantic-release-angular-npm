...
test('Validate ng-package.json file path', async (t) => {
  const cwd = temporaryDirectory();
  const npmrc = temporaryFile({ name: ".npmrc" });
  const packagePath = path.resolve(cwd, "package.json");
  ...
  const ngPackageFilePath = path.resolve(cwd, "ng-package.json");
  const ngPkg = {
  ...
  await fs.outputJson(ngPackageFilePath, ngPkg);

  await prepare(
    npmrc,
    { tarballDir: "tarball", ngPackageFilePath: ngPackageFilePath },
    {
      cwd,
      env: {},
      stdout: t.context.stdout,
  ...
  //Verify if ng-package.json file exists
  t.true(await fs.pathExists(path.resolve(cwd, ngPackageFilePath)));
  // Verify the logger has been called with ng-package.json file path
  t.deepEqual(t.context.log.args[0], ["Write version %s to package.json in %s", "1.0.0", cwd]);
  t.deepEqual(t.context.log.args[1], ["Executing ng-packagr from %s", ngPackageFilePath]);
});
